...
def align():
    ...
    parser.add_argument('-o', '--output',
                        action='store', dest='o', default='flair.aligned',
    ...
    if subprocess.call([args.sam, 'view', '-h', '-Sb', '-@', args.t, args.o + '.sam'],
                       stdout=open(args.o + '.unsorted.bam',
                                   'w')):  # calls samtools view, exit if an error code that != 0 results
    ...
    subprocess.call([sys.executable, path + 'bin/bam2Bed12.py', '-i', args.o + '.bam'],
                    stdout=open(args.o + '.bed', 'w'))
...
def collapse_range(corrected_reads='', aligned_reads=''):
    parser = argparse.ArgumentParser(description='flair-collapse parse options',
                                     usage='python flair.py collapse-range -g genome.fa -r reads.bam -q '
                                           '<query.psl>|<query.bed> [options]')
    ...
    parser.add_argument('-f', '--gtf', default='', action='store', dest='f',
                        help='GTF annotation file, used for renaming FLAIR isoforms to annotated isoforms '
    ...
    parser.add_argument('-b', '--bedtools', action='store', dest='b', default='bedtools',
                        help='bedtools executable path, provide if promoter regions specified and bedtools '
                             'is not in $PATH')
    parser.add_argument('-sam', '--samtools', action='store', dest='sam', default='samtools',
                        help='samtools executable path if not in $PATH')
...
def collapse(genomic_range='', corrected_reads=''):
    ...
    if args.p:
        ...
        subprocess.call([sys.executable, path + 'bin/psl_reads_from_bed.py',
                         args.temp_dir + tempfile_name + 'promoter_intersect.bed',
                         args.q, precollapse])
...
def quantify(isoform_sequences=''):
    ...
    parser.add_argument('--quality', type=int, action='store', dest='quality', default=1, help='''minimum MAPQ of read assignment to an isoform. If using salmon, all alignments are
        used (1)''')
    ...
    parser.add_argument('--trust_ends', default=False, action='store_true', dest='trust_ends',
                        help='specify if reads are generated from a long read method with minimal fragmentation')
    parser.add_argument('--temp_dir', default='', action='store', dest='temp_dir', help='''directory to put temporary files. use "./" to indicate current directory
        (default: python tempfile directory)''')
    args, unknown = parser.parse_known_args()
    if unknown:
    ...
    try:
        ...
        sys.stderr.write('codecs import error. Please pip install codecs. Exiting.\n')
        sys.exit(1)
    ...
    count_matrix.write("ids\t%s\n" % "\t".join(["_".join(x[:3]) for x in sam_data]))

    features = sorted(list(count_data.keys()))
    for f in features:
        count_matrix.write("%s\t%s\n" % (f, "\t".join(str(x) for x in count_data[f])))
...
def diffExp(counts_matrix=''):
    ...
    scripts_bin = path + "bin/"
    run_de = scripts_bin + "deFLAIR.py"
    de_command = [sys.executable, '-W ignore', run_de, '--filter', str(args.e), '--threads',
                 str(args.t), '--outDir', args.o, '--matrix', args.q]
    if args.of:
        de_command += ['-of']

    subprocess.call(de_command)
    return


def diffSplice(isoforms='', counts_matrix=''):
    parser = argparse.ArgumentParser(description='flair-diffSplice parse options',
                                     usage='python flair.py diffSplice -i isoforms.bed|isoforms.psl -q '
                                           'counts_matrix.tsv [options]')
    parser.add_argument('diffExp')
    required = parser.add_argument_group('required named arguments')
    if not isoforms:
